...
def _connect_mcp_oauth(doc) -> dict:
	...
	url = mcp_oauth.build_authorize_url(
		...
		# connect time, never stored, so a Custom URL row and every preset that
		# declares none send exactly the request they always did.
		extra_params=catalog.authorize_params_of((doc.get("preset") or "").strip()) or None,
	)
