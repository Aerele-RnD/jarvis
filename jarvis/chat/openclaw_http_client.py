...
def _post(url: str, headers: dict, body: dict, *, stream: bool):
	try:
		...
		raise OpenclawUnreachableError(f"self-hosted openclaw request failed: {e}") from e
...
def _check_status(r) -> None:
	...
	if r.status_code != 200:
		...
		raise OpenclawUnreachableError(f"self-hosted openclaw returned HTTP {r.status_code}: {snippet}")


def _empty_reply_error() -> OpenclawUnreachableError:
	return OpenclawUnreachableError(
		"self-hosted openclaw returned an empty reply (is an LLM configured and reachable on your openclaw?)"
	)
...
def _one_shot(url: str, headers: dict, body: dict) -> Iterator[dict[str, Any]]:
	...
	try:
		...
		raise OpenclawUnreachableError(f"self-hosted openclaw returned an unparseable response: {e}") from e
...
def _streamed(url: str, headers: dict, body: dict) -> Iterator[dict[str, Any]]:
	...
	try:
		...
		if not acc.strip():
			raise OpenclawUnreachableError(f"self-hosted openclaw stream failed: {e}") from e
	finally:
		r.close()
