...
class TestInstalledAppsSync(FrappeTestCase):
    ...
    def test_unconfigured_admin_skips(self):
        ...
        with patch.object(ias, "_admin_configured", return_value=False), \
                patch("frappe.enqueue") as enq:
            ias.after_migrate()
        enq.assert_not_called()
    ...
    def test_never_blocks_migrate(self):
        with patch.object(ias, "_current_apps", side_effect=RuntimeError("boom")), \
    ...
    def test_sync_via_admin_stamps_snapshot_on_send(self):
        ...
        with patch.object(type(settings), "_resolve_llm_secret_for_push",
                          return_value="sk-test"), \
             patch("jarvis.admin_client.post_update_llm_creds",
                   return_value={"action": "restart"}) as post, \
             patch("jarvis.jarvis.doctype.jarvis_settings.jarvis_settings"
                   "._is_applying_result", return_value=False), \
             patch("jarvis.jarvis.doctype.jarvis_settings.jarvis_settings"
    ...
    def test_failed_send_does_not_stamp(self):
        ...
        with patch.object(type(settings), "_resolve_llm_secret_for_push",
                          return_value="sk-test"), \
             patch("jarvis.admin_client.post_update_llm_creds",
                   side_effect=admin_client.AdminAuthError("nope")), \
             patch("jarvis.jarvis.doctype.jarvis_settings.jarvis_settings"
                   "._commit_terminal_sync_status"), \
