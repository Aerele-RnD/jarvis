...
class TestComputeMateriality(unittest.TestCase):
	def test_basic_tiers(self):
		m = compute_materiality(
			benchmark_value=10_000_000, percentage=5, engagement_risk_level="Medium", rounding_step=1000
		)
...
class TestScrutinyHelpers(unittest.TestCase):
	...
	def test_pack_new_analytical_rules_frozen_contract(self):
		...
		self.assertEqual(
			(ev["domain"], ev["status"], ev["kind"], ev["severity"]),
		...
		self.assertEqual(
			(sw["domain"], sw["status"], sw["kind"], sw["severity"]),
		...
		for rid in (
			"LS-ADVANCE-FROM-CUSTOMER-MAT",
			"LS-ADVANCE-TO-SUPPLIER-MAT",
			"LS-SIGN-FLIP-YOY",
			"COMP-MSME-OVERDUE",
		):
...
class TestRunScrutinyLive(unittest.TestCase):
	...
	def setUpClass(cls):
		...
		if not company:
			raise unittest.SkipTest("no Company on this site — live GL assertions need one")
	...
	def test_new_kinds_not_unsupported(self):
		res = run_scrutiny(
			rule_pack=_inline_pack(
				_rule(
					"T-ADV", "advance_in_party", {"party_type": "Supplier", "side": "debit", "min_amount": 0}
				),
				...
				_rule(
					...
					{"variance_pct": 20, "min_amount": 0},
					domain="analytical-review",
				),
				_rule(
					"T-SWING", "pl_yoy_swing", {"swing_pct": 25, "min_amount": 0}, domain="analytical-review"
				),
			)
		)
		...
		res = run_scrutiny(
			rule_pack=_inline_pack(
				_rule(
					"T-ADV-MAT",
					"advance_in_party",
					{"party_type": "Supplier", "side": "debit", "min_amount": "$materiality:bs_balance"},
				),
			)
		)
	...
	def test_msme_overdue_evaluates_and_states_basis(self):
		res = run_scrutiny(
			rule_pack=_inline_pack(
				_rule("T-MSME", "msme_overdue", {"days": 45}, domain="compliance", severity="warning")
			)
		)
		self.assertEqual(res["skipped_unsupported"], [])
		self.assertEqual(res["skipped_not_evaluable"], [])
	...
	def test_reproducible_hit_sets(self):
		pack = _inline_pack(
			_rule("T-ADV", "advance_in_party", {"party_type": "Supplier", "side": "debit", "min_amount": 0}),
			_rule(
				"T-ADV-C", "advance_in_party", {"party_type": "Customer", "side": "credit", "min_amount": 0}
			),
	...
	def test_bundled_pack_audit_domain_with_engagement(self):
		...
		if "LS-SIGN-FLIP-YOY" in not_evaluable:
			row = next(s for s in res["skipped_not_evaluable"] if s["rule_id"] == "LS-SIGN-FLIP-YOY")
	...
	def test_bundled_pack_analytical_review_domain(self):
		...
		evaluated_clean = {
			r["rule_id"] for r in _load_pack("scrutiny-pack")["rules"] if r["domain"] == "analytical-review"
		} - touched
...
class TestRunScrutinyPermissionGate(FrappeTestCase):
	...
	def test_restricted_user_denied_before_any_query(self):
		with _as(USER_NO_GRANTS), self.assertRaises(frappe.PermissionError):
			run_scrutiny(
				rule_pack=_trivial_pack(),
				company=SCRUTINY_COMPANY_A,
