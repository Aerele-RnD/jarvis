...
# ---------------------------------------------------------------------------
def _resolve_scope(
	company: str | None, fiscal_year: str | None, from_date: str | None, to_date: str | None
) -> dict:
	if not company:
...
def _k_voucher_pct_of_ledger(rule, scope, materiality):
	...
	for r in rows:
		...
		if in_band:
			hits.append(
				{
					"ref_doctype": r.voucher_type,
					"ref_name": r.voucher_no,
					"amount": float(r.v_amt),
	...
	return [
		{
			...
			"ref_name": r.voucher_no,
			"amount": float(r.amt),
			"note": f"Cash {direction} {float(r.amt):.2f} via {r.account} (> {float(threshold):.0f})",
		}
		for r in rows
	]
...
def _k_dormant_party_ledger(rule, scope, materiality):
	...
	net_expr = (
		"coalesce(sum(g.credit),0)-coalesce(sum(g.debit),0)"
	...
	mid = frappe.utils.add_days(
		scope["from_date"], int((frappe.utils.date_diff(scope["to_date"], scope["from_date"])) / 2)
	)
	if window == "last_half":
		act_from = mid
	...
	label = {
		"full": "no transactions during the year",
		"last_half": "no transactions in the last six months",
		"no_payment": "no payment during the year",
	}.get(window, window)
	return [
		{
			"ref_doctype": party_type,
			"ref_name": r.party,
			"amount": float(r.bal),
			"note": f"{party_type} {r.party}: balance {float(r.bal):.2f}, {label}",
		}
		for r in rows
	]
...
def _k_advance_in_party(rule, scope, materiality):
	...
	net_expr = (
		"coalesce(sum(g.debit),0)-coalesce(sum(g.credit),0)"
		if side == "debit"
	...
	rows = frappe.db.sql(
		f"""select g.party, {net_expr} bal
            ...
            order by g.party""",
		{**scope, "party_type": party_type, "min_amount": float(min_amount)},
		as_dict=True,
	)
	...
	return [
		{
			...
			"ref_name": r.party,
			"amount": float(r.bal),
			"note": f"{party_type} {r.party}: {label} {float(r.bal):.2f} as of {scope['to_date']}",
		}
		for r in rows
	]
...
def _has_gl_rows(company: str, from_date: str | None, to_date: str) -> bool:
	...
	row = frappe.db.sql(
		...
		{"company": company, "chk_from": from_date, "chk_to": to_date},
		as_dict=True,
	)[0]
	return int(row.n) > 0
...
def _k_sign_flip_yoy(rule, scope, materiality):
	...
	if not py:
		return _NotEvaluable(
			"no Fiscal Year ends before the scope period; prior-year closing balances unavailable"
		)
	...
	rows = frappe.db.sql(
		"""select a.name account,
           ...
           order by a.name""",
		{**scope, "py_end": py["py_end"], "min_amount": float(min_amount)},
		as_dict=True,
	)
	hits = []
	for r in rows:
		...
		hits.append(
			{
				...
				"amount": abs(cy_net),
				"note": (
					f"{r.account}: closing balance flipped {frm}->{to} "
...
def _k_msme_overdue(rule, scope, materiality):
	...
	return [
		{
			"ref_doctype": r.v_type,
			"ref_name": r.v_no,
			"amount": float(r.outstanding),
			"note": (
				f"MSME supplier {r.party}: {float(r.outstanding):.2f} outstanding on {r.v_no} "
				f"since {r.originated} (> {days} days at {scope['to_date']}; "
				f"MSME detection basis: {basis})"
			),
		}
		for r in rows
	]
...
def _k_expense_variance_yoy(rule, scope, materiality):
	...
	if not py:
		return _NotEvaluable(
			"no Fiscal Year ends before the scope period; prior-year comparatives unavailable"
		)
	if not _has_gl_rows(scope["company"], py["py_start"], py["py_end"]):
		return _NotEvaluable(
			f"no GL rows in prior fiscal year {py['name']} ({py['py_start']}..{py['py_end']})"
		)
	params = {**scope, **py}
	rev = frappe.db.sql(
			...
			hits.append(
				{
					...
					"amount": cy_amt,
					"note": (
						f"{r.account}: {cy_ratio:.2f}% of revenue vs {py_ratio:.2f}% in "
			...
			hits.append(
				{
					...
					"amount": abs(delta),
					"note": (
						f"{r.account}: CY {cy:.2f} vs PY {py_amt:.2f} in {py['name']} "
...
def run_scrutiny(
	...
	for rule in pack.get("rules", []):
		...
		if hits is None or isinstance(hits, _NotEvaluable):
			...
			reason = (
				hits.reason
				if isinstance(hits, _NotEvaluable)
				else "materiality-bound threshold unresolved (no engagement_config)"
			)
			skipped_not_evaluable.append(
				{"rule_id": rid, "reason": reason, "statement": rule.get("statement")}
			)
			continue
		...
		for h in hits[:max_findings_per_rule]:
			findings.append(
				{
					"rule_id": rid,
					"assure_rule": rule.get("assure_rule"),
					"severity": sev,
