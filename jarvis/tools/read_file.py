...
def read_file(
    ...
    payload so a huge file doesn't blow the turn's context.
    """
...
def _read_image(content: bytes, ext: str) -> dict:
    ...
    out["note"] = (
        ...
        "tool — to analyse what the image shows, it needs to be given to a vision "
        "model as an image input, not read here."
    )
    return out
