...
def get_context(context):
	...
	# MCP connectors (MCP_CONNECTORS_PLAN.md P3): the Custom URL policy, shipped
	# up front so the SPA can read it with no round trip.
	# `connectors_api.list_connectors` returns the same flag for any caller
	# that only has the API (a stale boot cache, e.g.).
