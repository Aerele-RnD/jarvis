  ...
  <div class="jv-settings-body">
    ...
    <section class="jv-mon-card">
      ...
      <div v-else-if="!applicable" class="jv-mon-note">
        Connection details apply to multi-model (proxy) setups. This tenant runs a single model (direct), so there is no proxy connection to report.
      </div>
      <template v-else>
        <div class="jv-mon-kv">
          <span>Status</span>
          <b :class="conn.auth_present ? 'jv-ok' : 'jv-warn'">{{ conn.auth_present ? "Connected" : "Not connected" }}</b>
        </div>
...
const applicable = computed(() => {
  const c = conn.value || {}
  return !!(c.auth_present || c.default_model || c.oauth_expires_at || (Array.isArray(c.profile_ids) && c.profile_ids.length))
})

async function load() {
  if (!isSystemManager) { loading.value = false; return }
  loading.value = true
  err.value = false
