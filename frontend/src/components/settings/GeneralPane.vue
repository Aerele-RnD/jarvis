<template>
	<div class="jv-settings-body">
		...
		<div class="jv-set-row">
			<span>Provider</span><b>{{ ui.llm_provider || "—" }}</b>
		</div>
		<div class="jv-set-row">
			<span>Auth mode</span><b>{{ ui.llm_auth_mode || "—" }}</b>
		</div>
		<div class="jv-set-row">
			<span>Status</span
			><b :style="{ color: connected ? 'var(--green)' : 'var(--text-3)' }">{{
				statusLabel
			}}</b>
		</div>
		...
		<div class="jv-set-row">
			<span
				>Confirm before changes<br /><span class="jv-set-sub"
					>Ask before creating, updating, or submitting in this chat. Deletes, cancels,
					amends, and emails always ask, even with this off.</span
				></span
			>
			<button
				class="jv-switch"
				:class="{ on: !convAutoApply }"
				@click="onToggleAutoApply"
				:disabled="!hasConversation"
				role="switch"
				:aria-checked="String(!convAutoApply)"
				:title="
					convAutoApply
						? 'Auto mode - changes apply without asking'
						: 'Confirm each change before it runs'
				"
			>
				<span class="jv-switch-knob"></span>
			</button>
		</div>
		<div v-if="!hasConversation" class="jv-set-row" style="padding-top: 0">
			<span class="jv-set-sub">Open a conversation to change this — it's set per chat.</span>
		</div>
		<div v-else-if="autoApplyNote" class="jv-set-row" style="padding-top: 0">
			<span style="font-size: 11px; color: var(--amber); font-weight: 500">{{
				autoApplyNote
			}}</span>
		</div>
		<div class="jv-set-row">
			<span
				>Show tool activity<br /><span class="jv-set-sub"
					>Show the live tool steps + input/output above each reply. The tools count
					&amp; time always show below.</span
				></span
			>
			<button
				class="jv-switch"
				:class="{ on: showActivityDetail }"
				@click="setActivityDetail(!showActivityDetail)"
				role="switch"
				:aria-checked="String(showActivityDetail)"
				title="Show the tool/skill activity under each answer"
			>
				<span class="jv-switch-knob"></span>
			</button>
		</div>
		<div class="jv-set-row">
			...
			<button
				...
				@click="toggleNotify"
				role="switch"
				:aria-checked="String(notifyEnabled)"
				title="Browser notification when a reply finishes in a background tab"
			>
				<span class="jv-switch-knob"></span>
			</button>
		</div>
		...
		<div
			class="jv-set-sec"
			style="margin-top: 18px; display: flex; align-items: center; gap: 7px"
		>
			Token usage <span class="jv-est">est.</span>
		</div>
		<div class="jv-set-row">
			<span>This chat</span><b>{{ usage ? fmtTokens(usage.chat_tokens) : "—" }}</b>
		</div>
		<div class="jv-set-row">
			<span>{{ usage ? usage.month_label : "This month" }}</span
			><b>{{ usage ? fmtTokens(usage.month_tokens) : "—" }}</b>
		</div>
		<div class="jv-set-row">
			<span>All time</span><b>{{ usage ? fmtTokens(usage.total_tokens) : "—" }}</b>
		</div>
		...
		<div class="jv-set-row">
			<span
				>Delete all chat history<br /><span class="jv-set-sub"
					>Every conversation and message, permanently. Macros and skills stay.</span
				></span
			>
			<button
				class="jv-btn jv-btn--sm jv-btn-danger"
