	...
	<div class="jv-settings-body">
		<div class="jv-mon">
			<section class="jv-mon-card">
				<h3>Status</h3>
				<div class="jv-mon-kv">
					<span>Mode</span><b>{{ config.proxy_active ? "Proxy" : "Direct" }}</b>
				</div>
				<div class="jv-mon-kv">
					<span>Sync</span><b>{{ sync.last_sync_status || "—" }}</b>
				</div>
				<div v-if="sync.last_sync_at" class="jv-mon-kv">
					<span>Last sync</span><b>{{ sync.last_sync_at }}</b>
				</div>
			</section>
			...
			<section class="jv-mon-card">
				<h3>Active pool</h3>
				<div class="jv-mon-kv">
					<span>Preset</span><b>{{ config.preset || "Custom" }}</b>
				</div>
				<div class="jv-mon-kv">
					<span>Routing</span><b>{{ config.routing_mode || "failover" }}</b>
				</div>
				<ol class="jv-mon-models">
					<li v-for="(m, i) in config.models || []" :key="i">
						{{ m.provider }} · {{ m.model }}
						<span class="jv-mon-tag">{{
							i === 0 ? "runs every turn" : "backup"
						}}</span>
					</li>
				</ol>
			</section>
			...
			<section class="jv-mon-card">
				<h3>
					Usage <span class="jv-mon-sub">· {{ usage.period || "current period" }}</span>
				</h3>
				<div v-if="usageError" class="jv-mon-note">
					Usage is unavailable right now.
					<button type="button" class="jv-mon-retry" @click="loadAll">Retry</button>
				</div>
