	...
	<div class="flex items-center">
		...
		<Popover placement="bottom-end">
			<template #target="{ togglePopover }">
				<Button v-if="isDefault" label="Sort" iconLeft="bar-chart-2" @click="togglePopover()" />
				<Button v-else :label="fieldLabel" class="rounded-l-none" @click="togglePopover()" />
			</template>
			<template #body="{ close }">
				<div class="my-2 min-w-60 rounded-lg bg-surface-modal p-2 shadow-2xl ring-1 ring-black ring-opacity-5">
					<FormControl
						type="select"
						label="Sort by"
						:options="sortOptions"
					...
					<div class="mt-2 flex justify-end border-t pt-2">
						<Button variant="ghost" label="Clear Sort" class="!text-ink-gray-5" @click="reset(close)" />
					</div>
				</div>
			</template>
		</Popover>
		<Button v-if="!isDefault" variant="ghost" icon="x" :tooltip="'Reset sort'" @click="reset()" />
	</div>
</template>
...
function toggleDir() {
	emit("update:sort", { field: props.sort.field, dir: props.sort.dir === "asc" ? "desc" : "asc" })
}
...
function reset(close) {
	emit("update:sort", { field: props.defaultSort.field || "", dir: props.defaultSort.dir || "" })
	if (close) close()
}
</script>
