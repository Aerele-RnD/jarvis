	...
	<div class="flex items-center">
		<Popover placement="bottom-end">
			<template #target="{ togglePopover }">
				<Button label="Filter" iconLeft="filter" :class="activeCount ? 'rounded-r-none' : ''" @click="togglePopover()">
			...
			<template #body>
				<div class="my-2 min-w-72 rounded-lg bg-surface-modal p-2 shadow-2xl ring-1 ring-black ring-opacity-5">
					<div
					...
					<div v-else class="mb-3 flex flex-col gap-2">
						<div v-for="(def, i) in activeDefs" :key="def.key" class="flex items-center gap-2">
							<div class="w-13 shrink-0 pl-2 text-end text-base text-ink-gray-5">
								{{ i === 0 ? "Where" : "And" }}
							</div>
							<div class="min-w-[110px] text-base text-ink-gray-8">{{ def.label }}</div>
							...
							<div class="flex-1">
								...
								/>
								<div v-else-if="def.type === 'daterange'" class="flex items-center gap-1">
									<DatePicker
										class="!min-w-[110px]"
										placeholder="From"
		...
		<Button v-if="activeCount" icon="x" class="rounded-l-none border-l" :tooltip="'Clear all filters'" @click="clearAll" />
	</div>
</template>
