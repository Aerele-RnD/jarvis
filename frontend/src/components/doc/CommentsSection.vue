<template>
	<div>
		<div class="mb-4 flex items-center gap-2">
			...
			<Badge v-if="comments.length" variant="subtle" theme="gray" :label="String(comments.length)" />
		</div>

		<div v-if="comments.length" class="flex flex-col">
			<div v-for="c in comments" :key="c.name" class="flex gap-3 py-3">
				<Avatar size="md" :image="c.owner_image" :label="c.owner_name || c.owner" class="shrink-0" />
				<div class="min-w-0 flex-1">
					<div class="flex items-center gap-2 text-sm">
						<span class="font-medium text-ink-gray-8">{{ c.owner_name || c.owner }}</span>
						...
						<div v-if="isOwn(c) && editingId !== c.name" class="ml-auto">
							<Dropdown :options="menuFor(c)" placement="right">
								<template #trigger>
									<Button variant="ghost" icon="more-horizontal" class="!h-5 !w-5" />
								</template>
							</Dropdown>
						</div>
					</div>
