		...
		<div v-if="store.settingsOpen" class="jv-settings-overlay jv-root" :class="{ 'jv-dark': dark }" :style="paletteVars" @click.self="close">
			<div class="jv-settings">
				...
				<div class="jv-settings-nav">
					...
					<!-- ACCOUNT & BILLING (System Managers only) -->
					<template v-if="isSM">
						<div class="jv-settings-group">Account &amp; billing<span class="sm-tag">System Mgr</span></div>
						<button class="jv-settings-navitem" :class="{ on: section === 'plan' }" @click="go('plan')">
							<svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.7" stroke-linecap="round" stroke-linejoin="round"><rect x="1" y="4" width="22" height="16" rx="2" /><path d="M1 10h22" /></svg>
							<span>Plan &amp; billing</span>
						</button>
						<button class="jv-settings-navitem" :class="{ on: section === 'aimodels' }" @click="go('aimodels')">
							<svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.7" stroke-linecap="round" stroke-linejoin="round"><rect x="4" y="4" width="16" height="16" rx="2" /><rect x="9" y="9" width="6" height="6" /><path d="M9 1v3M15 1v3M9 20v3M15 20v3M20 9h3M20 14h3M1 9h3M1 14h3" /></svg>
							<span>AI models</span>
						</button>
						<button class="jv-settings-navitem" :class="{ on: section === 'connection' }" @click="go('connection')">
							<svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.7" stroke-linecap="round" stroke-linejoin="round"><path d="M5 12.55a11 11 0 0 1 14.08 0" /><path d="M1.42 9a16 16 0 0 1 21.16 0" /><path d="M8.53 16.11a6 6 0 0 1 6.95 0" /><path d="M12 20h.01" /></svg>
							<span>Connection</span>
						</button>
						<button class="jv-settings-navitem" :class="{ on: section === 'billing' }" @click="go('billing')">
							<svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.7" stroke-linecap="round" stroke-linejoin="round"><path d="M12 1v22" /><path d="M17 5H9.5a3.5 3.5 0 0 0 0 7h5a3.5 3.5 0 0 1 0 7H6" /></svg>
				...
				<div class="jv-settings-main">
					<div class="jv-settings-head">
						...
						<button class="jv-iconbtn" @click="close" title="Close" style="width:28px;height:28px;display:flex;align-items:center;justify-content:center;background:transparent;border:none;border-radius:7px;cursor:pointer;color:var(--text-3);">
...
// ACCOUNT & BILLING is gated on System Manager; ADMINISTRATION is gated on
// window.is_jarvis_admin (true for System Managers too — see design doc §2).
// Both are UX-only gates; the server enforces the real permission on every
// underlying endpoint regardless of what the client hides.
