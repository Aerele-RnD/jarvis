...
test("defaultSubscriptionModel: falls back to built-in defaults with no catalog", () => {
	assert.equal(defaultSubscriptionModel("openai"), "gpt-5.6-sol");
	assert.equal(defaultSubscriptionModel("unknown"), "gpt-5.6-sol");
	assert.equal(defaultSubscriptionModel(undefined), "gpt-5.6-sol");
});
...
test("defaultSubscriptionModel: a catalog overrides the built-in default", () => {
	...
	assert.equal(defaultSubscriptionModel("unknown", catalog), "gpt-5.6-sol");
});
...
test("subModelSuggestions: empty or missing payload yields the built-in fallback", () => {
	assert.equal(subModelSuggestions({}).openai[0], "gpt-5.6-sol");
	assert.equal(subModelSuggestions(undefined).openai[0], "gpt-5.6-sol");
});
