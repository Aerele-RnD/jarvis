<template>
	...
	>
		<template #body-content>
			...
			<div v-if="loading" class="py-8 text-center text-sm text-ink-gray-5">Loading people…</div>
			<template v-else>
				<!-- selected chips -->
				...
				<div v-else class="mt-2 max-h-64 overflow-y-auto">
					...
					>
						...
						<span class="flex min-w-0 flex-1 flex-col">
							<span class="truncate text-base text-ink-gray-8">{{ u.full_name || u.name }}</span>
		...
		<template #actions>
			<div class="flex items-center gap-2">
				<Button variant="solid" label="Save" :loading="saving" :disabled="loading" @click="save" />
