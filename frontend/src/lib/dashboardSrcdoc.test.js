...
test("full document with <head>: re-wrapped, CSP first, head/body inner preserved", () => {
	const out = buildSrcdoc(
		"<!DOCTYPE html><html><head><title>t</title></head><body>USER</body></html>",
		{}
	);
...
test("charset is always present (fragment or full doc, since we always wrap)", () => {
	assert.ok(
		buildSrcdoc("<html><head></head><body></body></html>", {}).includes(
			'<meta charset="utf-8">'
		)
	);
...
test("runtime script precedes user markup in all three cases", () => {
	for (const html of [
		...
		assert.ok(
			out.indexOf(RUNTIME_MARK) < out.indexOf("USER"),
			`runtime after markup for: ${html}`
		);
	}
});
...
test("theme attribute: set in all cases, replaced when already present", () => {
	...
	const b = buildSrcdoc('<html data-theme="light"><head></head><body></body></html>', {
		dark: true,
	});
