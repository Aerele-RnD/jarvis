	...
	while ((m = fromRe.exec(content))) {
		...
		if (braces) {
			for (const piece of braces[1].split(',')) {
				const original = piece.trim().replace(/^type\s+/, '').split(/\s+as\s+/)[0].trim()
...
export function frappeUIContentGlobs() {
	try {
		...
		while (queue.length) {
			...
			for (const { spec, names } of parseImports(content)) {
				...
				if (resolved === indexFile) {
					if (!names.length) throw new Error(`unresolvable barrel import in ${file}`)
		...
		for (const file of reachable) {
			...
			const glob =
				rel.length > 2
					? ['.', 'node_modules', 'frappe-ui', 'src', rel[0], rel[1], '**', GLOB_EXTS].join('/')
